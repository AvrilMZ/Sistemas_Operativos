...
sys_exofork(void)
{
	...
	newenv->priority =
	        curenv->priority;  // hijo hereda la prioridad del padre
...
// Changes the priority of the 'envid' to the one given.
static int
sys_setpriority(envid_t envid, int priority)
{
	struct Env *env;
	int r;
	if ((r = envid2env(envid, &env, 1)) < 0) {
	...
	// Mantener la prioridad dentro del rango.
	if (priority < 0 || priority > MAX_PRIORITY) {
		return -E_INVAL;
	}
	// Bajar la prioridad propia, pero no subirla.
	if (envid == curenv->env_id && priority >= env->priority) {
		return -E_INVAL;
	}
	// Otros procesos solo permitir bajar la prioridad.
	if (priority > env->priority) {
		return -E_INVAL;
	}
