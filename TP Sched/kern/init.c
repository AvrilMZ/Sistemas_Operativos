...
#define TESTED(x) (__builtin_strcmp(#x, STRING(TEST)) == 0)
	...
	extern struct Env *env_last_created;

	// 'yield' con prioridad baja
	ENV_CREATE(user_yield, ENV_TYPE_USER);
	env_last_created->priority = 2;

	// 'spin' con prioridad por defecto (MAX_PRIORITY)
	ENV_CREATE(user_spin, ENV_TYPE_USER);

	// Tests de syscalls de prioridad
	ENV_CREATE(user_prioself, ENV_TYPE_USER);
	ENV_CREATE(user_priofork, ENV_TYPE_USER);
#else
